...
variable "www_domain_name" {
  default = "www.drihosting.net"
}

// We'll also need the root domain (also known as zone apex or naked domain).
variable "root_domain_name" {
  default = "drihosting.net"
}
...
resource "aws_acm_certificate" "certificate" {
  // Use the alias to have certificate stored in us-east-1 to be able to assign to CF
  provider = "aws.acm"  
...
resource "aws_cloudfront_distribution" "www_distribution" {
  ...
  // Here we're ensuring we can hit this distribution using www.drihosting.net
